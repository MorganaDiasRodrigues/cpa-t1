...
def link_crawler(start_url, link_regex, robots_url=None, user_agent='wswp',
    ...
    while crawl_queue:
        ...
        if rp.can_fetch(user_agent, url):
            ...
            if depth == max_depth:
                print(f'Skipping {url} due to depth: {depth}')
                continue
            throttle.wait(url)
            ...
            for link in get_links(html):
                if not link_regex or re.match(link_regex, link):
...
# %%
link_crawler(start_url='''http://192.168.1.103:8000/places/default/index''',
             link_regex=None,
             max_depth=2,
             robots_url='''http://192.168.1.103:8000/places/static/robots.txt''')
